...
| Recompute an energy scan | The complete retained \(G_1\)–\(G_4\) numerical calculation is repeated | `run_arbitrary_N_energy_scan.m` with settings loaded from an archived MAT file |
...
|---|---|
| `N_delta_1loop_correction.zip` | MATLAB driver 1.3.0, post-processing 1.3.0, numerical engine 1.9.6, and the validation suite |
...
`chapter6_data_set.zip` into `chapter6_figures`.
...
| Heavy-mass decoupling figure | `decoupling.png` | Rendered image only in the current public archive; see the limitation below |
...
270 corresponding files present in the public code ZIP match their recorded
...
These are documentation files rather than executable numerical kernels.
Nevertheless, R99 checks the complete frozen manifest, so R99 will report a
baseline-manifest mismatch when rerun from the current reduced public ZIP. To
reproduce the recorded R99 pass exactly, restore those files from the original
frozen package with their original hashes. Keep this repository-level README
outside the frozen code ZIP so that it does not modify that manifest.
...
| File | SHA-256 |
|---|---|
| `N_delta_1loop_correction.zip` | `1b77e1251d44a6de619cc73eff48b16bb96dace4866755e12d4adb8710f1e8c5` |
...
| `chapter6_data_set.zip` | `daf400f18aa3a7af4647a3c6cb24d20bf2ccae2fdc8e5ede6c8b0ee136dd7e60` |
...
1. **The heavy-mass thesis curve is not fully reproducible from the current
   public files.** `chapter6_data_set.zip` includes `decoupling.png`, but it
   does not include the exact mass grid, signed \(\Delta T\) values, fitting
   mask, or the plotting script used for that image. The R06 validation test
   independently recomputes a three-mass heavy-field decoupling check at
   \(m_\Phi=100,300,1000\); it is not the same data set as the rendered thesis
   curve. The remaining thesis plots can be reconstructed from the supplied
   final MAT files.
...
The public archive is named `chapter6_data_set.zip`, while the older plotting
